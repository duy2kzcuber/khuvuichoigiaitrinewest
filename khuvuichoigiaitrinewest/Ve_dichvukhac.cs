...
{
    public partial class Ve_dichvukhac : Form
    {
        public Ve_dichvukhac()
        {
            InitializeComponent();
        }
        ...
        private bool checktrungma(string mave)
        {
            ...
            string sql = "Select count(*) From ve_dichvukhac where mave= '" + mave + "'";
        ...
        private void load_cbodv()
        {
            string sql = " Select * From dv_khac";
            DataTable dt = new DataTable();
            dt = Thuvien.Hienthi(sql);
            DataRow r = dt.NewRow();
            r["ten_dv"] = "Chọn dịch vụ cần thuê";
            r["ma_dv"] = "";
            dt.Rows.InsertAt(r, 0);
            cbo_tendv.DataSource = dt;
            cbo_tendv.DisplayMember = "ten_dv";
            cbo_tendv.ValueMember = "ten_dv";

        }
        private void Load_dgv()
        {
            string sql = "Select * From ve_dichvukhac";
            DataTable dt = new DataTable();
            dt = Thuvien.Hienthi(sql);
            dgv_vedichvu.DataSource = dt;
            dgv_vedichvu.Refresh();
        }
        private void btn_them_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string p_tendv = cbo_tendv.SelectedValue.ToString();
                ...
                if (p_tendv == "Chọn dịch vu cần tìm")
                {
                    MessageBox.Show("Bạn phải chọn dịch vụ");
                    cbo_tendv.Focus();
                    return;
                }
                ...
                string sql1 = "insert ve_dichvukhac Values(N'" + p_mave + "', N'" + p_tendv + "', N'" + p_tt + "', '" + p_tgban + "', '" + p_tgdong + "', ";
        ...
        private void cbo_tendv_SelectedIndexChanged(object sender, EventArgs e)
        {
            string p_tendv = cbo_tendv.Text.ToString();
            string sql = " Select * From dv_khac where ten_dv= N'" + p_tendv + "'";
        ...
        private void Ve_dichvukhac_Load(object sender, EventArgs e)
        {
            load_cbodv();
            Load_dgv();
        }

        private void dgv_vedichvu_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            int i = e.RowIndex;
            txt_mave.Text = dgv_vedichvu.Rows[i].Cells[1].Value.ToString();
            cbo_tendv.Text = dgv_vedichvu.Rows[i].Cells[0].Value.ToString();
            cbo_tt.Text = dgv_vedichvu.Rows[i].Cells[2].Value.ToString();
            dtp_tgban.Text = dgv_vedichvu.Rows[i].Cells[3].Value.ToString();
            dtp_tgdong.Text = dgv_vedichvu.Rows[i].Cells[4].Value.ToString();
            txt_gvnglon.Text = dgv_vedichvu.Rows[i].Cells[5].Value.ToString();
            txt_gvtreem.Text = dgv_vedichvu.Rows[i].Cells[6].Value.ToString();
            txt_sonl.Text = dgv_vedichvu.Rows[i].Cells[7].Value.ToString();
            txt_sote.Text = dgv_vedichvu.Rows[i].Cells[8].Value.ToString();
            txt_tongtien.Text = dgv_vedichvu.Rows[i].Cells[9].Value.ToString();
        }
        ...
        private void btn_xuat_Click(object sender, EventArgs e)
        {
            ...
            string sql = "Select * From ve_dicvukhac Where mave like '%" + p_mave + "%' and ten_dichvhu like N'%" + p_tendv + "%' and trangthai like  N'%" + p_tt + "%'";
            DataTable dt = Thuvien.Hienthi(sql);
            dgv_vedichvu.DataSource = dt;
            dgv_vedichvu.Refresh();
        }
